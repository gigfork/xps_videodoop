...
implements Mapper<Text, Text, IntWritable, Text>
{
	...
	public void conf_cache(JobConf conf)
	{
		this.conf=conf;
		try
		{
	...
	public void conf_normal(JobConf conf)
	{
		this.conf=conf;
		try
		{			
	...
	public void map(Text key, Text value,
		...
		try {		
			fitness = cft.getFitness(array);
			//System.err.println(value.toString()+"     " +   fitness + "  --   "+cft.getNumVeh());
			output.collect(new IntWritable(Integer.parseInt(key.toString())),new Text(fitness));//value.toString()+'\t'+fitness));
